...
This folder contains the Hermes dashboard plugin API that backs BITCH personal glyph sync.
...
The endpoint reads the current generated glyph artifact from:
...
So this folder is useful as the source package for the glyph plugin, but copying it to `$HERMES_HOME/plugins/bitch` is not enough to mount `/api/plugins/bitch/glyph/current` on current Hermes versions.
...
BITCH stays remote-only: it does not start Hermes locally and does not read `$HERMES_HOME` from the desktop client. The desktop app asks the authenticated remote Hermes dashboard for the artifact, and this plugin is the Hermes-side reader for that artifact.
