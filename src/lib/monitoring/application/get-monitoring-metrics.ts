...
import type { MonitoringMetrics } from '../domain/metrics'
import { asRecord, normalizeBeszelContainers, normalizeMonitoringMetrics, stringValue } from '../domain/normalize'
import type { MonitoringConfig, MonitoringRequestJson } from '../ports/monitoring-port'
...
async function fetchCollectionRecords(
  ...
  requestJson: MonitoringRequestJson
): Promise<unknown[]> {
...
async function fetchConfiguredSystem(config: MonitoringConfig, requestJson: MonitoringRequestJson): Promise<unknown> {
  try {
...
async function fetchSystemDetails(systemId: string, requestJson: MonitoringRequestJson): Promise<unknown | undefined> {
...
async function fetchVisibleSystem(requestJson: MonitoringRequestJson): Promise<unknown> {
  ...
  if (!system || !systemId) {
    throw new Error(
      `Beszel monitoring did not return any visible systems. The hub can be healthy while PocketBase records remain hidden from unauthenticated requests. ${monitoringAuthHint()}`
    )
  }
...
export async function fetchMonitoringMetrics(
  ...
  requestJson: MonitoringRequestJson = requestMonitoringJson
  ...
  const [statsResult, detailsResult, containersResult] = await Promise.allSettled([
    ...
    fetchCollectionRecords(
      BESZEL_COLLECTIONS.containers,
      {
        fields: BESZEL_CONTAINER_FIELDS,
  ...
  const containerRecords = containersResult.status === 'fulfilled' ? containersResult.value : []
  ...
  const containers = normalizeBeszelContainers(containerRecords, metrics.memory.totalBytes)
