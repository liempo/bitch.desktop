...
import { createHermesRemoteFilesAdapter } from '../../../hermes/files/adapters/hermes-remote-files-adapter'
import { listRemoteDirectory } from '../../../hermes/files/application/list-remote-directory'
import { resolveRemoteFileDataUrl, resolveRemoteFileText } from '../../../hermes/files/application/resolve-file-preview'
import { createUnavailableLocalFilesPort } from '../../../hermes/files/ports/local-files-port'
...
describe('Hermes files lane module shape', () => {
  it('publishes the Hermes files API from the new lane while preserving transitional re-exports', () => {
    ...
    expect(hermesFiles.createRemoteDirectory).toBeTypeOf('function')
    expect(hermesFiles.deleteRemotePath).toBeTypeOf('function')
    expect(hermesFiles.readRemoteManagedFileDataUrl).toBeTypeOf('function')
    expect(hermesFiles.uploadRemoteFile).toBeTypeOf('function')
    expect(hermesFiles.writeRemoteFileDataUrl).toBeTypeOf('function')
